...
function addNewsMessageToContainer(message, container, addToTop = true) {
  ...
  // Formatuj timestamp
  let formattedTimestamp = message.ts || 'N/A';
  try {
    if (message.ts) {
    ...
    console.warn("Nie można sformatować timestamp:", e);
  }
  
  // Formatuj tagi
  let tagsHtml = '';
  if (message.interpretation_tags && Array.isArray(message.interpretation_tags)) {
    message.interpretation_tags.forEach(tag => {
      tagsHtml += `<span class="news-label">${tag}</span>`;
    });
  }
  
  // Formatuj entity info
  let entityInfoHtml = '<p><strong>Entities:</strong></p>';
  if (message.extracted_entities && Array.isArray(message.extracted_entities) && message.extracted_entities.length > 0) {
    entityInfoHtml += '<ul class="entity-list">';
    message.extracted_entities.forEach(entity => {
      entityInfoHtml += `<li>
        <span class="entity-name">${entity.normalized_name || entity.text || 'N/A'}</span>
        <span class="entity-type"> (${entity.type || 'Unknown'})</span>
      </li>`;
    });
    entityInfoHtml += '</ul>';
  } else {
    entityInfoHtml += '<p>No entities found</p>';
  }
  ...
  const messageHtml = `
    <div class="news-item ${impactClass} fade-in" data-news-id="${message.news_id}">
      ...
      <div class="news-content">
        ...
        <p><strong>Interpretation:</strong> ${message.interpretation || 'N/A'}</p>
        ${entityInfoHtml}
      </div>
...
async function loadInitialMessages() {
  ...
  try {
    // Zmieniono limit z 20 na 10
    const response = await window.fetchWithAuth('/api/news?limit=10');
    ...
    if (!data.news || !Array.isArray(data.news) || data.news.length === 0) {
      ...
      if (newsContainer.querySelector('.news-item') === null) {
        newsContainer.innerHTML = '<p style="text-align: center; padding: 20px; color: var(--current-text-muted);">Brak wiadomości</p>';
      }
      return;
    }
