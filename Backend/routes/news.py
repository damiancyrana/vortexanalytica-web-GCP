...
router = APIRouter(
    prefix="/api/news", tags=["news"], dependencies=[Depends(get_current_active_user)]
)
...
async def get_news(
    ...
    logger.info(
        f"Pobieranie wiadomości dla użytkownika z sesji: {current_user_session.get('email')}"
    )
        ...
        try:
            ...
            while not await request.is_disconnected():
                try:
                    ...
                    logger.error(
                        f"Błąd podczas obsługi kolejki SSE: {e}", exc_info=True
                    )
                    break
        finally:
        ...
        try:
            ...
            while not await request.is_disconnected():
                try:
                    ...
                    logger.error(
                        f"Błąd podczas obsługi kolejki krytycznej SSE: {e}",
                        exc_info=True,
                    )
                    break
        finally:
            ...
            logger.info(
                f"Zamknięto krytyczny strumień SSE dla użytkownika: {user_email}"
            )
...
async def cleanup_old_messages(
    ...
    user_id = current_user_session.get("user_id")
    logger.warning(
        f"Użytkownik {user_id} uruchomił czyszczenie starych wiadomości (max_age: {max_age_hours}h)"
    )
    ...
    # Wykonaj czyszczenie
    deleted_count = await news_service.cleanup_old_messages(
